...
package cn.zhuatech.knowledgegraph;

import cn.zhuatech.knowledgegraph.service.WorkspaceService;
...
class WorkspaceServiceTests {
    ...
    void returnsReviewableDomainResult() {
        var request = new WorkspaceService.RunRequest("库存锁定接口", "项目知识关系治理", 75, true, "演示上下文");
    ...
    void blocksDirectAutomationWithoutHumanReview() {
        var request = new WorkspaceService.RunRequest("库存锁定接口", "项目知识关系治理", 60, false, "");
        var result = service.run(request);
